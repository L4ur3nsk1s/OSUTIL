...
setup_ubuntu() {
    ...
    pd login ubuntu --shared-tmp -- env DISPLAY=:1 apt update -y || { echo "Failed to update Ubuntu packages."; exit 1; }
    pd login ubuntu --shared-tmp -- env DISPLAY=:1 apt upgrade -y || { echo "Failed to upgrade Ubuntu packages."; exit 1; }

    # Set timezone
    timezone=$(getprop persist.sys.timezone)
    pd login ubuntu --shared-tmp -- env DISPLAY=:1 ln -sf /usr/share/zoneinfo/$timezone /etc/localtime
    ...
    echo "$username:$password" | pd login ubuntu --shared-tmp -- env DISPLAY=:1 chpasswd || { echo "Failed to set user password."; exit 1; }
