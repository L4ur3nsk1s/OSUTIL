...
# Load zsh plugins explicitly (fix for some environments)
...
# ---------- proot-distro setup with optional XFCE inside Ubuntu ----------
...
# Change repo to Lithuania mirror
...
EOF
  ...
  # Copy and run ubuntu base setup script inside proot
  proot-distro login ubuntu -- mkdir -p /root
  proot-distro login ubuntu -- sh -c "cat > /root/.setup_ubuntu.sh" < "$setup_script"
  proot-distro login ubuntu -- bash /root/.setup_ubuntu.sh || {
    warn "Failed to run Ubuntu setup script inside proot"
  }
  rm -f "$setup_script"

  # Ask to install XFCE + VNC inside Ubuntu
  read -rp "Do you want to install XFCE desktop environment and VNC server inside Ubuntu? (y/N): " xfce_choice
  if [[ "$xfce_choice" =~ ^[Yy]$ ]]; then
    info "Installing XFCE and VNC inside Ubuntu..."
...
# Create simple startxfce script
...
EOF
    ...
    # Copy and run xfce setup script inside proot
    proot-distro login ubuntu -- mkdir -p /root
  ...
  # Create the Termux shortcut to enter Ubuntu as the user
...
EOF
  ...
  info "You can enter Ubuntu as user '$ubuntu_user' by running: startubuntu"
}
